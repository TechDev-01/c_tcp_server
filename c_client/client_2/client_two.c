...
int main(){
   ...
   * and the third parameter is the IP address of the server_client
  */
  //Set the server_client structure
  server_client.sin_family = AF_INET;
  server_client.sin_port = htons(9002);
  inet_pton(AF_INET, "192.168.1.110", &server_client.sin_addr); //Connect to a specific IP address
  
  if (bind(client_socket, (struct sockaddr*)&server_client, sizeof(server_client)) < 0) {
    perror("Error binding socket to IP\n");
}
